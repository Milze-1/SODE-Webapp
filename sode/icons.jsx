// SODE — icon set (Lucide-style, 24px, 2px round stroke), pillar marks, brand logo.
// Exports to window: Icon, BrandMark, Wordmark, PILLARS, pillarOf
...
function Icon({ name, size = 22, stroke = 2, color, className, style }) {
...
// Brand mark: sprout rising over an arc, in a rounded navy tile
function BrandMark({ size = 40, radius, navy = '#1e2a52', on = '#fff' }) {
...
function Wordmark({ color = 'var(--ink)', sub = 'var(--muted)', center = false, scale = 1 }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: center ? 'center' : 'flex-start', lineHeight: 1 }}>
      <span style={{ fontSize: 9.5 * scale, letterSpacing: '.22em', fontWeight: 700, color: sub, textTransform: 'uppercase' }}>The School Of</span>
      <span style={{ fontSize: 19 * scale, fontWeight: 800, color, letterSpacing: '-.01em', marginTop: 2 * scale }}>Daniels &amp; Esthers</span>
    </div>
  );
}

const PILLARS = [
  { key: 'spiritual', name: 'Spiritual', short: 'Spiritual', icon: 'sprout', color: 'var(--p-spiritual)', raw: '#1e2a52', verse: '“But Daniel resolved not to defile himself.” — Daniel 1:8' },
  { key: 'career', name: 'Career & Calling', short: 'Career', icon: 'briefcase', color: 'var(--p-career)', raw: '#3a4e86', verse: '“For such a time as this.” — Esther 4:14' },
  { key: 'business', name: 'Business', short: 'Business', icon: 'store', color: 'var(--p-business)', raw: '#5c6b8c', verse: '“Ten times better than all.” — Daniel 1:20' },
  { key: 'character', name: 'Character', short: 'Character', icon: 'compass', color: 'var(--p-character)', raw: '#828ba0', verse: '“Whoever is faithful in little…” — Luke 16:10' },
];
const pillarOf = (k) => PILLARS.find(p => p.key === k) || PILLARS[0];

Object.assign(window, { Icon, BrandMark, Wordmark, PILLARS, pillarOf });
