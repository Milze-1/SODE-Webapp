// SODE — shared UI components. Exports to window.
const { useState, useEffect, useRef } = React;

/* ---------- Avatar (monogram) ---------- */
function Avatar({ name = '', size = 40, tone = 'navy', img, style }) {
  ...
  return (
    <div style={{
      ...
      fontWeight: 700, fontSize: size * 0.36, letterSpacing: '.01em', overflow: 'hidden', ...style,
    }}>
...
/* ---------- Pillar chip ---------- */
function PillarChip({ pillar, size = 'md', solid = false }) {
  ...
  return (
    <span className="chip" style={{ height: sm ? 22 : 26, fontSize: sm ? 11 : 12, paddingLeft: 7 }}>
      <span style={{ display: 'inline-flex', color: p.color }}><Icon name={p.icon} size={sm ? 12 : 14} stroke={2.2} /></span>
      {p.short}
    </span>
  );
}
...
/* ---------- Progress ring ---------- */
function ProgressRing({ value = 0, size = 52, stroke = 5, color = 'var(--navy)', track = 'var(--surface-2)', children, delay = 120 }) {
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flex: 'none' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {children != null ? children : <span className="tnum" style={{ fontSize: size * 0.28, fontWeight: 700 }}>{Math.round(value * 100)}</span>}
      </div>
    </div>
  );
}
...
/* ---------- Progress bar ---------- */
function ProgressBar({ value = 0, color = 'var(--navy)', height = 8, track = 'var(--surface-2)', delay = 120 }) {
  const [v, setV] = useState(0);
  useEffect(() => { const t = setTimeout(() => setV(value), delay); return () => clearTimeout(t); }, [value, delay]);
  return (
    <div style={{ background: track, borderRadius: 999, height, overflow: 'hidden' }}>
      <div style={{ width: `${Math.max(0, Math.min(1, v)) * 100}%`, height: '100%', background: color, borderRadius: 999, transition: 'width 1s cubic-bezier(.22,1,.36,1)' }} />
    </div>
  );
}

/* ---------- Status pill ---------- */
function StatusPill({ status = 'ontrack', size = 'md' }) {
  const map = {
...
/* ---------- Points badge ---------- */
function PointsBadge({ value, size = 'md', onClick }) {
  const sm = size === 'sm';
  return (
...
/* ---------- Stat callout ---------- */
function Stat({ value, label, sub, align = 'left' }) {
  return (
...
function SectionHead({ title, action, onAction }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10 }}>
      ...
      {action && <button onClick={onAction} style={{ fontSize: 13, fontWeight: 600, color: 'var(--navy)', display: 'inline-flex', alignItems: 'center', gap: 2 }}>{action}<Icon name="chevronright" size={14} stroke={2.4} /></button>}
    </div>
  );
}

/* ---------- Bottom sheet ---------- */
function Sheet({ open, onClose, title, children, footer, dim = true }) {
  if (!open) return null;
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 50, display: 'flex', alignItems: 'flex-end' }}>
      {dim && <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(17,20,28,.40)', animation: 'sode-fade .2s ease' }} />}
      <div style={{
        ...
        {title && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 18px 6px' }}>
            ...
            <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)' }}><Icon name="x" size={18} /></button>
          </div>
        )}
        <div className="noscroll" style={{ overflowY: 'auto', padding: '4px 18px 18px' }}>{children}</div>
        {footer && <div style={{ padding: '12px 18px calc(12px + env(safe-area-inset-bottom))', borderTop: '1px solid var(--line)', background: 'var(--bg)' }}>{footer}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', left: 14, right: 14, bottom: 86, zIndex: 60, display: 'flex', justifyContent: 'center', pointerEvents: 'none' }}>
      <div style={{
        ...
        padding: '11px 16px', borderRadius: 14, background: offline ? '#fff' : 'var(--navy)',
        color: offline ? 'var(--ink)' : '#fff', boxShadow: 'var(--sh-pop)',
        border: offline ? '1px solid var(--line-2)' : 'none',
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      {pieces.map((_, i) => {
        ...
        return <span key={i} style={{
          ...
          background: cols[i % cols.length], opacity: .9,
...
function Celebrate({ data, onClose }) {
  if (!data) return null;
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 80, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 28, background: 'rgba(245,246,248,.86)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)', animation: 'sode-fade .2s ease' }}>
      <Confetti />
      <div style={{ animation: 'sode-pop .5s cubic-bezier(.22,1.4,.4,1)' }}>
        <BrandMark size={84} radius={26} />
      </div>
      ...
      {data.points != null && (
        <div style={{ marginTop: 18, display: 'inline-flex', alignItems: 'center', gap: 7, padding: '8px 16px', borderRadius: 999, background: 'var(--navy)', color: '#fff', fontWeight: 800 }}>
          <Icon name="zap" size={18} stroke={2.4} color="#fff" /><span className="tnum" style={{ fontSize: 17 }}>+{data.points} points</span>
        </div>
      )}
      <button className="btn btn-primary btn-lg" style={{ marginTop: 28, minWidth: 200 }} onClick={onClose}>{data.cta || 'Amen — continue'}</button>
      {data.secondary && (
        <button className="btn btn-ghost" style={{ marginTop: 10, minWidth: 200 }} onClick={() => { onClose(); data.secondary.onClick && data.secondary.onClick(); }}>
          {data.secondary.icon && <Icon name={data.secondary.icon} size={17} stroke={2.2} />}{data.secondary.label}
        </button>
      )}
    </div>
  );
      ...
      {options.map(o => {
        ...
        return (
          <button key={v} onClick={() => onChange(v)} style={{
            ...
            color: active ? 'var(--ink)' : 'var(--muted)', background: active ? '#fff' : 'transparent',
            boxShadow: active ? 'var(--sh-sm)' : 'none', transition: 'all .15s ease', whiteSpace: 'nowrap',
          }}>{label}</button>
        );
      })}
    </div>
...
/* ---------- Fields ---------- */
function Field({ label, hint, children, saved }) {
  return (
    <label style={{ display: 'block', marginBottom: 16 }}>
      {label && <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 7 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink-2)' }}>{label}</span>
        {saved && <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted)', display: 'inline-flex', alignItems: 'center', gap: 3 }}><Icon name="check" size={12} stroke={2.6} /> Saved</span>}
      </div>}
      {children}
...
const inputStyle = {
  ...
  background: 'var(--surface)', padding: '0 14px', fontSize: 15, fontFamily: 'var(--font)', color: 'var(--ink)', outline: 'none',
};
function TextInput({ value, onChange, placeholder, type = 'text', multiline, rows = 3, icon }) {
  if (multiline) {
...
function OptionChips({ options, value, onChange, multi = false, columns }) {
  ...
  return (
    <div style={{ display: columns ? 'grid' : 'flex', gridTemplateColumns: columns ? `repeat(${columns}, 1fr)` : undefined, flexWrap: columns ? undefined : 'wrap', gap: 8 }}>
      {options.map(o => {
        const v = typeof o === 'string' ? o : o.value;
        const label = typeof o === 'string' ? o : o.label;
        ...
        return (
          <button key={v} onClick={() => toggle(v)} style={{
            display: 'inline-flex', alignItems: 'center', justifyContent: columns ? 'flex-start' : 'center', gap: 7,
            padding: '0 14px', height: 44, borderRadius: 999, fontSize: 14, fontWeight: 600,
            background: sel ? 'var(--navy)' : 'var(--surface)', color: sel ? '#fff' : 'var(--ink)',
            border: sel ? '1px solid var(--navy)' : '1px solid var(--line-2)', transition: 'all .14s ease',
          }}>
...
function Toggle({ on, onChange }) {
  return (
    <button onClick={() => onChange(!on)} style={{
      width: 46, height: 28, borderRadius: 999, padding: 3, background: on ? 'var(--navy)' : 'var(--line-2)',
      transition: 'background .18s ease', flex: 'none',
    }}>
      <span style={{ display: 'block', width: 22, height: 22, borderRadius: '50%', background: '#fff', boxShadow: 'var(--sh-sm)', transform: on ? 'translateX(18px)' : 'none', transition: 'transform .18s cubic-bezier(.22,1,.36,1)' }} />
    </button>
  );
}

/* ---------- Empty state ---------- */
function EmptyState({ icon = 'sprout', title, body, cta, onCta }) {
  return (
    <div style={{ textAlign: 'center', padding: '36px 24px' }}>
      ...
      {body && <p style={{ fontSize: 14, color: 'var(--muted)', marginTop: 7, lineHeight: 1.5, maxWidth: 270, marginLeft: 'auto', marginRight: 'auto' }}>{body}</p>}
      {cta && <button className="btn btn-primary" style={{ marginTop: 18 }} onClick={onCta}><Icon name="plus" size={18} stroke={2.4} color="#fff" />{cta}</button>}
    </div>
  );
}

Object.assign(window, {
  Avatar, PillarChip, ProgressRing, ProgressBar, StatusPill, PointsBadge, Stat, SectionHead,
  Sheet, Toast, Confetti, Celebrate, Segmented, Field, TextInput, OptionChips, Toggle, EmptyState,
});
